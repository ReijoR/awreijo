...
public class Populate {
    ...
    private static void createTables(Connection con){
        dropCreate(con,"car","(id int primary key not null auto_increment,make varchar(32), regcity varchar(32))");
        dropCreate(con,"passanger","(id int primary key not null auto_increment, car_id int,name varchar(32), psgtype varchar(32))");       
        try(Statement stm=con.createStatement()){
            stm.addBatch("insert into car values(null,'Blechfire Runabout','Tampere')");
            stm.addBatch("insert into car values(null,'Batmobil','Helsinki')");
            stm.addBatch("insert into car values(null,'DeLorean','Tampere')");
            stm.addBatch("insert into car values(null,'The Love Bug','Helsinki')");
            stm.executeBatch();
            stm.close();
        }
        catch(SQLException ex){
